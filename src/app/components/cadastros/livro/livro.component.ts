...
export class LivroComponent {
  ...
  constructor(private formBuilder: FormBuilder,
    ...
    private iconRegistry: MatIconRegistry,) {
      ...
      this.formGroup = formBuilder.group({
        ...
        autor: [(livro && livro.autor) ? livro.preco : '', 
              Validators.compose([Validators.required])],
  ...
  cadastrar() {
    ...
    if (this.formGroup.valid) {
      ...
      operacao.subscribe({
        next: () => this.router.navigateByUrl('/livro'),
        error: (error: HttpErrorResponse) => {
          console.log('Falha ao Cadastrar' + JSON.stringify(error));
  ...
  excluir() {
    if (this.formGroup.valid) {
      ...
      if (estado.id != null) {
        this.livroService.delete(estado).subscribe({
          next: () => {
  ...
  tratarErros(error: HttpErrorResponse) {
    if (error.status === 400) {
      ...
      if (error.error?.errors) {
        ...
        alert(error.error?.message || 'Erro ao cadastrar livro.');
  ...
  errorMessages: {[controlName: string]: {[errorName: string] : string}} = {
    ...
    preco: {
      required: 'Insira o preço do livro.',
      apiError: ' ' 
    },
    autor: {
    ...
    ano_publicacao: {
      required: 'Insira o ano de publicação.',
      apiError: ' ' 
    },
    editora: {
      required: 'Insira o nome da editora .',
    },
    isbn: {
      required: 'Insira o número de identificação do livro.',
      apiError: ' ' 
    }
  }
